...
`;
  ...
  try {
    ...
    console.error("Error generating plan:", apiError || error.message);

    if (error.response?.status === 403) {
      return res.status(503).json({
        error:
          "AI service key is invalid or suspended. Update GOOGLE_API_KEY in server environment variables.",
      });
    }
