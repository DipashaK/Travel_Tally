...
export const sanitizeText = (text, removeColons = true) =>
  text?.replace(/\*/g, "").replace(removeColons ? /:/g : "", "") || "";
  ...
  export const parseAiPlan = (plan, destination) => {
    ...
    const aboutPlace = extractSection(plan, `About\\s+${destination}`, "Top Activities");
  ...
  export const formatItinerary = (itinerary) => {
    ...
    return days
      .map((dayBlock, dayIndex) => {
        ...
        return (
          <div key={dayIndex} className="p-4 border rounded-xl bg-white shadow-md mt-4">
            ...
            {sections.map((section, i) => (
              <div key={i} className="bg-blue-50 p-4 rounded-lg mb-4 shadow-sm">
                <h4 className="text-md font-semibold text-gray-700 mb-2 flex items-center gap-2">
                  <span>{timeIcons[section.period] || "🕒"}</span> {section.period}
                </h4>
